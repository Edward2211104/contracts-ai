...
class AskBodyBasic(BaseModel):
    ...
    Campos:
      - question: str
      - contractText: texto completo del contrato (opcional pero recomendado)
...
async def llm_ask_basic(body: AskBodyBasic):
    ...
    Comportamiento:
      - Si viene contractText => usa motor experto en contratos
        (answer_contract_question) con ese texto.
      - Si NO viene contractText => usa modo genérico de LLM
        (call_lm_studio).
    """
    ...
    # 1) Camino experto: tenemos texto de contrato
    if has_contract:
    ...
    # 2) Camino genérico: sin contrato, respuesta general
    ...
    system_prompt = (
        ...
        "If the user does not provide any contract text, explain clearly what "
        "information or clauses you would need from the contract to answer in detail."
    )
